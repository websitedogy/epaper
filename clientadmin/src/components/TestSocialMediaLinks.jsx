...
const TestSocialMediaLinks = () => {
  ...
  const renderSocialMediaLinks = (customization) => {
    ...
    // Define style configurations for social links
    const styleConfigs = {
      circle: {
        name: "Circle Icons",
        platforms: [
          ...
          .map(platform => {
            ...
            switch (platform.shape) {
              ...
              case "rounded":
                ...
                if (typeof platform.bgColor === 'string' && platform.bgColor.startsWith('linear-gradient')) {
                  iconContainerStyle.background = platform.bgColor;
                }
                iconStyle = {
                  color: platform.color,
                  fontSize: "18px"
  ...
  const renderSocialMediaIcons = (customization) => {
    ...
    // Define style configurations (same as in SocialMediaIconsSettings)
    const styleConfigs = {
      style1: {
        name: "Circle Icons",
    ...
    // Get the current style configuration
    const currentStyle = styleConfigs[selectedStyleKey] || styleConfigs.style1;
    ...
    // Filter platforms that have URLs and render icons
    const filteredPlatforms = currentStyle.platforms.filter(platform => socialMediaUrls[platform.key]);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1>Test Social Media Links Display</h1>
...
export default TestSocialMediaLinks;